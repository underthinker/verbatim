...
- [ ] Overlay never takes focus (verified on KDE, the spike 1 regression case) and respects reduced-motion.
  (2026-07-09: **macOS closed, reduced-motion closed.** `scripts/verify-overlay.sh` drives a real session against the live window server on the default fake backends and asserts the pill maps on screen, that Verbatim never becomes frontmost, and that the pill never becomes the main window.
  The check is calibrated against a deliberately `focusable(true)` build, which fails it - the first draft passed that build, because `AXFocused` reads `false` for every window of an inactive app and so stays green on an overlay that did become key; `AXMain` plus the app's own `AXFocusedWindow` are the properties that actually flip.
  Reduced-motion moved from a per-selector kill list in `overlay.css`, which the sweep and the bar transitions had already escaped in principle, to a single `prefers-reduced-motion: reduce` media query over the pill and every descendant, so a later animation cannot escape it.
  The structural half - waveform giving way to a static level meter - stays in `Overlay.tsx`, since it swaps elements rather than stilling them.
  KDE Plasma 6 and GNOME remain open and are the case this criterion exists to guard; on Linux `focusable(false)` reaches only `gtk_window_set_accept_focus`, leaving `gtk_window_set_focus_on_map` at its default, which is the first suspect if KWin steals focus on map.)
